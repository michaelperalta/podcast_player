...
if len(selected_filters) > 0:
    ...
    st.subheader(str(len(episodes)) + ' episodes match your criteria')
    
    select_random_episode = st.checkbox('Select Random Episode',value=True)
        
    if select_random_episode:
    
        random_episode = st.button('Select Again')
        random_episode = True
        ...
        selected_episode = st.selectbox('Select Episode',options=episodes['title'])
